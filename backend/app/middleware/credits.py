...
class CreditChecker:
    ...
    async def check_user_credits(self, user_id: str, credits_needed: int = 1) -> dict[str, Any]:
        ...
        if not limit_check.can_optimize:
            raise ValueError(
                f"Insufficient credits: {limit_check.remaining_free_optimizations} remaining, {credits_needed} needed"
            )
...
async def verify_credits_manual(
    user_id: str, credits_needed: int = 1, supabase_client=None
) -> bool:
    """
