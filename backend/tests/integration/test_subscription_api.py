...
def mock_current_user_admin():
    """Mock admin user."""
    return {"id": "admin_123", "email": "admin@example.com", "name": "Admin User", "role": "admin"}
...
class TestSubscriptionStatus:
    ...
    def test_get_subscription_status_no_subscription(
        self, mock_current_user, mock_subscription_service
    ):
        """Test subscription status when no active subscription."""
        # Arrange
        mock_subscription_service.get_subscription_status.return_value = Mock(
            has_active_subscription=False, tier_id=None, analyses_remaining=10, can_use_service=True
        )
    ...
    def test_get_subscription_status_service_error(
        self, mock_current_user, mock_subscription_service
    ):
...
class TestUpdateSubscription:
    ...
    def test_update_subscription_not_found(self, mock_current_user, mock_subscription_service):
        ...
        # Arrange
        mock_subscription_service.get_subscription_details.side_effect = ValueError(
...
class TestCancelSubscription:
    ...
    def test_cancel_subscription_immediate_success(
        self, mock_current_user, mock_subscription_service, mock_stripe
    ):
    ...
    def test_cancel_subscription_at_period_end_success(
        ...
        mock_stripe.Subscription.modify.assert_called_once_with(
            "sub_stripe_123", cancel_at_period_end=True
        )
        mock_subscription_service.cancel_subscription.assert_called_once_with(
            "sub_123", immediate=False
        )
    ...
    def test_cancel_subscription_wrong_user(self, mock_current_user, mock_subscription_service):
        ...
        # Act
        response = client.post(
            "/api/subscriptions/sub_123/cancel", headers={"Authorization": "Bearer test_token"}
        )
...
class TestGetUsageHistory:
    ...
    def test_get_usage_history_success(self, mock_current_user, mock_supabase):
        ...
        # Act
        response = client.get(
            "/api/subscriptions/history", headers={"Authorization": "Bearer test_token"}
        )
    ...
    def test_get_usage_history_with_limit(self, mock_current_user, mock_supabase):
        ...
        # Act
        response = client.get(
            "/api/subscriptions/history?limit=10", headers={"Authorization": "Bearer test_token"}
        )
...
class TestCreateCheckoutSession:
    ...
    def test_create_checkout_session_success(
        self, mock_current_user, mock_stripe, mock_pricing_config, mock_supabase
    ):
        """Test successful checkout session creation."""
        # Arrange
        checkout_request = {"tier_id": "flow_pro"}
    ...
    def test_create_checkout_session_invalid_tier(self, mock_current_user, mock_pricing_config):
        ...
        checkout_request = {"tier_id": "invalid_tier"}
...
class TestBrazilianMarket:
    ...
    def test_brl_currency_handling(
        self, mock_current_user, mock_stripe, mock_pricing_config, mock_supabase
    ):
