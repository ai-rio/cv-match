...
def mock_database_complete():
    ...
    with patch('app.services.subscription_service.get_supabase_client') as mock_supabase, \
        ...
        def mock_table_chain(table_name):
            ...
            if table_name == "subscriptions":
                ...
                table_mock.select().eq().execute.return_value = Mock(data=[])  # No existing subscription
...
def mock_services_complete():
    ...
    with patch('app.api.subscriptions.subscription_service') as mock_sub_service, \
...
class TestCompleteSubscriptionFlow:
    ...
    @pytest.mark.asyncio
    async def test_complete_subscription_lifecycle(self, mock_user, mock_stripe_complete, mock_database_complete, mock_services_complete):
    ...
    @pytest.mark.asyncio
    async def test_subscription_upgrade_flow(self, mock_user, mock_stripe_complete, mock_database_complete, mock_services_complete):
        ...
        mock_pricing.get_tier.side_effect = lambda tier_id: {
            "flow_basic": Mock(is_subscription=True, stripe_price_id="price_basic", name="Flow Basic", analyses_per_month=15, rollover_limit=5),
            "flow_pro": Mock(is_subscription=True, stripe_price_id="price_pro", name="Flow Pro", analyses_per_month=60, rollover_limit=30),
            "flow_business": Mock(is_subscription=True, stripe_price_id="price_business", name="Flow Business", analyses_per_month=200, rollover_limit=100)
        }.get(tier_id)
    ...
    @pytest.mark.asyncio
    async def test_subscription_with_rollover(self, mock_user, mock_stripe_complete, mock_database_complete, mock_services_complete):
    ...
    @pytest.mark.asyncio
    async def test_subscription_payment_failure_and_recovery(self, mock_user, mock_stripe_complete, mock_database_complete, mock_services_complete):
        ...
        webhook_result = await webhook_service.process_invoice_payment_succeeded(success_invoice_data)
        assert webhook_result["success"] is True
    ...
    @pytest.mark.asyncio
    async def test_immediate_vs_period_end_cancellation(self, mock_user, mock_stripe_complete, mock_database_complete, mock_services_complete):
    ...
    @pytest.mark.asyncio
    async def test_usage_tracking_with_subscription_limits(self, mock_user, mock_stripe_complete, mock_database_complete, mock_services_complete):
    ...
    @pytest.mark.asyncio
    async def test_webhook_error_handling_and_recovery(self, mock_user, mock_stripe_complete, mock_database_complete, mock_services_complete):
        ...
        webhook_client.table().select().eq().execute.side_effect = Exception("Database connection failed")
