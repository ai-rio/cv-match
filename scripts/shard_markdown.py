...
class MarkdownChunk:
    ...
    def __str__(self):
        return (
            f"Chunk {self.chunk_number}: {self.title} (~{self.token_estimate} tokens)"
        )
...
class MarkdownSharder:
    ...
    def create_chunks(self, sections: List[Dict]) -> List[MarkdownChunk]:
        ...
        for section in sections:
            ...
            if (
                current_chunk_tokens + section_tokens + context_tokens > self.max_tokens
    ...
    def _create_chunk(
        self, number: int, headers: List[Dict], content: str, tokens: int
    ) -> MarkdownChunk:
    ...
    def _split_large_section(
        self, section: Dict, start_chunk_number: int
    ) -> List[MarkdownChunk]:
...
"""
    ...
    def _create_index(
        self, chunks: List[MarkdownChunk], output_dir: Path, source_file: Path
    ):
...
"""
        ...
        for chunk in chunks:
            context = (
                " > ".join(chunk.headers_context[-2:])
                if len(chunk.headers_context) > 1
                else chunk.headers_context[0]
                if chunk.headers_context
                else "N/A"
            )
